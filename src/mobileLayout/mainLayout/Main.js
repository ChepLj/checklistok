...
function MainMbLo() {
    ...
    useEffect(() => {
        if (submitPage) {
            navigate('/submit', { state: { title: state, refDoc: ref } });
        }
    }, [state]);
    ...
    return (
        <>
            <div className={style.app}>
                <Header parentRef={[...ref]} time={timeState} callBack={(value) => setRef(value)} />
                {(function (data) {
                    if (data.level === 'Group')
                        return <GroupBtn title={state} parentRef={[...ref]} callBack={(value) => setRef(value)} />;
                    if (data.level === 'Area')
                        return (
                            <AreaBtn
                                title={state}
                    ...
                    // if (data.level === 'Equipment') return <EquipmentBtn title={state} />;
                    if (data.level === 'Equipment') {
                        return (submitPage = true);
                    }
